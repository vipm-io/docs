...
| 2026-06-18 | Filled out the vipm.toml command-reference entries for `lock`, `add`, and `remove` | ✅ | Replaced the `vipm lock` placeholder with a full entry (opt-in lock creation, `--check` exit code `2`, `--best-effort`, dev-deps always locked) and added `vipm add` / `vipm remove` sections plus Quick Look rows in `docs/cli/command-reference.md`. All entries use the generated `_generated/commands/*.md` snippets and link to the new [Lock Files](../vipm-toml/lock-files.md) page. |
| 2026-06-22 | Documented CLI liveliness timeout environment variables | ✅ | Added the Desktop and network liveliness timeout defaults, reset conditions, disable behavior, and relationship to total operation timeouts. |
