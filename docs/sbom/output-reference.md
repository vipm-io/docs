...
"tools": {
  "components": [
    ...
    },
    {
      ...
      "name": "LabVIEW",
      "version": "26.3.0",
      "description": "LabVIEW 2026 Q3 (64-bit)",
      "properties": [
        { "name": "vipm:labview:bitness", "value": "64" }
      ]
    },
    {
      "type": "application",
...
- **`vipm-cli`** is always listed and reports its own build version.
- **`VIPM Desktop`** appears only when the SBOM contains VI packages and an installed VIPM Desktop's version could be read.
- **`LabVIEW`** appears only when the SBOM was generated by scanning a LabVIEW project (`.lvproj`) against a concrete installed LabVIEW. The entry reports that installation's version at the greatest available specificity, a human-readable display name in `description`, and the bitness via the `vipm:labview:bitness` property. SBOMs generated from manifest inputs (`vipm.toml`, `.vipc`, `.dragon`, `vipm.lock`) carry no LabVIEW tool entry.
- **`NI Package Manager`** appears only when the SBOM contains NI packages and an installed NI Package Manager's version could be read (`nipkg` is available on Windows).

Tools that are not installed or did not take part in producing the SBOM's contents are omitted — there are no placeholder entries. Publisher names are read from the installed products where possible and normalized to canonical forms (`JKI`, `National Instruments`).
...
For `.lvproj` inputs, the CLI also invokes the LabVIEW linker to trace subVI call chains automatically — discovering dependencies that are loaded at runtime but not directly listed in the project file. The traversal recurses to fixpoint so the SBOM reflects the full transitive closure.
